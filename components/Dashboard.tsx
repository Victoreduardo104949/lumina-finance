...
import { Eye, EyeOff, TrendingUp, Wallet, Target, Activity, ArrowUpRight, ArrowDownLeft, HeartPulse, Flame, Plus, CreditCard, ChevronRight, Edit2, ArrowRightLeft, Trash2 } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({
  transactions, accounts, vaults, fixedExpenses, debts,
  privacyMode, togglePrivacy, userStats, onAddVault,
  onEditVault, onTransferVault, onDeleteVault
}) => {
        ...
        <div className="glass-card p-6 rounded-[2.5rem] border-2 border-slate-50 dark:border-slate-800">
          ...
          <div className="space-y-4">
            {vaults.length === 0 ? (
              ...
              vaults.map(v => (
                <div key={v.id} className="group/vault space-y-1.5 relative">
                  ...
                  <div className="absolute inset-0 bg-white/90 dark:bg-slate-900/90 flex items-center justify-center gap-4 opacity-0 group-hover/vault:opacity-100 transition-opacity rounded-xl -m-2 p-2">
                    <button onClick={() => onTransferVault(v)} className="p-2 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-transform hover:scale-110">
                      <ArrowRightLeft size={16} />
                    </button>
                    <button onClick={() => onEditVault(v)} className="p-2 text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg transition-transform hover:scale-110">
                      <Edit2 size={16} />
                    </button>
                    <button onClick={() => onDeleteVault(v.id)} className="p-2 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-lg transition-transform hover:scale-110">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
